...
#define SET_GPR(N,EXPR)							\
  ((void)(((N) == 31) ? setPC++ : 0), regs.regs_R[N] = (EXPR))
/*
#define SET_GPR(N,EXPR)							\
...
#elif defined(TARGET_MYISA)
/*	
#undef SET_GPR(N,EXPR)
  ...
  while (TRUE)
    {
      ...
      if (op == NA)
	fatal("bogus opcode detected @ 0x%08p, inst: 0x%08x", regs.regs_PC, inst);
      setPC = 0;
      regs.regs_R[MD_REG_PC] = regs.regs_PC;
